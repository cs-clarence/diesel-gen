          ...
          match ord {
            ...
            PrismaMigrationOrderBy::AppliedStepsCountAsc => {
              if idx == 0 {
                ...
                q = q
                  .then_order_by(_prisma_migrations::applied_steps_count.asc());
              }
            }
            ...
            PrismaMigrationOrderBy::AppliedStepsCountDesc => {
              if idx == 0 {
                ...
                q = q.then_order_by(
                  _prisma_migrations::applied_steps_count.desc(),
                );
              }
            }
          }
...
pub mod email_address_verification_code {
  #[derive(
    Clone,
    Debug,
    diesel::Queryable,
  ...
  impl EmailAddressVerificationCode {
      ...
      Conn: diesel_async::AsyncConnection<Backend = diesel::pg::Pg> + Send,
    {
      ...
      diesel::update(email_address_verification_codes::table)
        .filter(email_address_verification_codes::id.eq(id))
        .set((email_address_verification_codes::email_address_id
          .eq(email_address_id),))
        .returning(EmailAddressVerificationCode::as_returning())
        .get_result::<EmailAddressVerificationCode>(conn)
    }
          ...
          match ord {
            ...
            EmailAddressVerificationCodeOrderBy::IdDesc => {
              if idx == 0 {
                ...
                q =
                  q.then_order_by(email_address_verification_codes::id.desc());
              }
            }
            ...
            EmailAddressVerificationCodeOrderBy::ValueAsc => {
              if idx == 0 {
                ...
                q = q
                  .then_order_by(email_address_verification_codes::value.asc());
              }
            }
            ...
            EmailAddressVerificationCodeOrderBy::ValueDesc => {
              if idx == 0 {
                ...
                q = q.then_order_by(
                  email_address_verification_codes::value.desc(),
                );
              }
            }
            ...
            EmailAddressVerificationCodeOrderBy::ExpiresAtAsc => {
              if idx == 0 {
                q = q
                  .order_by(email_address_verification_codes::expires_at.asc());
              } else {
                q = q.then_order_by(
            ...
            EmailAddressVerificationCodeOrderBy::CreatedAtAsc => {
              if idx == 0 {
                q = q
                  .order_by(email_address_verification_codes::created_at.asc());
              } else {
                q = q.then_order_by(
...
pub mod email_address {
  ...
  impl EmailAddress {
    pub fn update<'a, Conn>(
      ...
      Output = Result<EmailAddress, diesel::result::Error>,
    > + Send
         + 'a
    where
      Conn: diesel_async::AsyncConnection<Backend = diesel::pg::Pg> + Send,
...
pub mod refresh_token {
  ...
  impl RefreshToken {
    pub fn update<'a, Conn>(
      ...
      Output = Result<RefreshToken, diesel::result::Error>,
    > + Send
         + 'a
    where
      Conn: diesel_async::AsyncConnection<Backend = diesel::pg::Pg> + Send,
...
pub mod user {
  ...
  impl UserCursor {
      ...
      ) -> users::BoxedQuery<'b, diesel::pg::Pg>,
    {
      ...
      let create_query = || {
        let mut q =
          users::table.order_by(users::created_at.desc()).into_boxed();
    ...
    pub fn paginate_reversed<'a, Conn>(
      after: Option<&'a UserCursor>,
      before: Option<&'a UserCursor>,
      limit: Option<usize>,
      offset: Option<usize>,
    ...
    where
      Conn: diesel_async::AsyncConnection<Backend = diesel::pg::Pg> + Send,
    {
      UserCursor::paginate_reversed_extend(
        after,
        before,
        limit,
        offset,
    ...
    pub fn paginate_reversed_extend<'a, F, Conn>(
      after: Option<&'a UserCursor>,
      before: Option<&'a UserCursor>,
      limit: Option<usize>,
      offset: Option<usize>,
    ...
    pub fn has_next_reversed<'a, Conn>(
      cursor: &'a UserCursor,
      conn: &'a mut Conn,
    ) -> impl std::future::Future<Output = Result<bool, diesel::result::Error>>
         + Send
    ...
    where
      Conn: diesel_async::AsyncConnection<Backend = diesel::pg::Pg> + Send,
    {
      UserCursor::has_next_reversed_extend(cursor, |q| q, conn)
    }

    pub fn has_next_reversed_extend<'a, F, Conn>(
      cursor: &'a UserCursor,
      extend: F,
      conn: &'a mut Conn,
    ) -> impl std::future::Future<Output = Result<bool, diesel::result::Error>>
    ...
    pub fn has_previous_reversed<'a, Conn>(
      cursor: &'a UserCursor,
      conn: &'a mut Conn,
    ) -> impl std::future::Future<Output = Result<bool, diesel::result::Error>>
         + Send
    ...
    where
      Conn: diesel_async::AsyncConnection<Backend = diesel::pg::Pg> + Send,
    {
      UserCursor::has_previous_reversed_extend(cursor, |q| q, conn)
    }

    pub fn has_previous_reversed_extend<'a, F, Conn>(
      cursor: &'a UserCursor,
      extend: F,
      conn: &'a mut Conn,
    ) -> impl std::future::Future<Output = Result<bool, diesel::result::Error>>
    ...
    where
      Conn: diesel_async::AsyncConnection<Backend = diesel::pg::Pg> + Send,
    {
      UserCursor::paginate_reversed_with_soft_deleted_extend(
        after,
        before,
        limit,
        offset,
    ...
    pub fn paginate_reversed_with_soft_deleted_extend<'a, F, Conn>(
      after: Option<&'a UserCursor>,
      before: Option<&'a UserCursor>,
      limit: Option<usize>,
      offset: Option<usize>,
    ...
    pub fn has_next_reversed_with_soft_deleted<'a, Conn>(
      cursor: &'a UserCursor,
      conn: &'a mut Conn,
    ) -> impl std::future::Future<Output = Result<bool, diesel::result::Error>>
         + Send
    ...
    where
      Conn: diesel_async::AsyncConnection<Backend = diesel::pg::Pg> + Send,
    {
      UserCursor::has_next_reversed_with_soft_deleted_extend(
        cursor,
        |q| q,
        conn,
      )
    ...
    pub fn has_next_reversed_with_soft_deleted_extend<'a, F, Conn>(
      cursor: &'a UserCursor,
      extend: F,
      conn: &'a mut Conn,
    ) -> impl std::future::Future<Output = Result<bool, diesel::result::Error>>
    ...
    pub fn has_previous_reversed_with_soft_deleted<'a, Conn>(
      cursor: &'a UserCursor,
      conn: &'a mut Conn,
    ) -> impl std::future::Future<Output = Result<bool, diesel::result::Error>>
         + Send
    ...
    where
      Conn: diesel_async::AsyncConnection<Backend = diesel::pg::Pg> + Send,
    {
      UserCursor::has_previous_reversed_with_soft_deleted_extend(
        cursor,
        |q| q,
        conn,
      )
